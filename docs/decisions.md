...
- 초기 세 Room은 평일에만 운영하며, 주말에는 열리지 않는다.
- Room은 weekday_open, weekend_open으로 평일·주말 운영 여부를 저장한다. 두 값이 모두 false인 Room은 허용하지 않는다.
- 자정을 넘는 Room은 자정 전에는 당일 운영 여부를, 자정 후에는 전날 운영 여부를 기준으로 연다.
